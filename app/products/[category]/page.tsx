...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  return <>
    ...
    <section className="section"><div className="content-wrap"><div className="catalog-section-heading"><div><h2>{category.name} Catalog</h2><p>Review the published model references and available technical data. Confirm site conditions and final configuration with our team before ordering.</p></div><p className="catalog-page-status">Page {page} of {pageCount}</p></div><div className="catalog-grid">{visibleProducts.map((product) => <article className="card" key={product.id}>{product.heroImage ? <Image className="card-image" src={product.heroImage} alt={product.gallery[0]?.alt ?? `${product.name} product view`} width={720} height={540} sizes="(max-width: 560px) 92vw, (max-width: 800px) 45vw, (max-width: 1100px) 30vw, 22vw" /> : <div className="placeholder-image">Approved product image pending.</div>}<h3>{product.name}</h3><p>{product.shortDescription}</p><ul className="spec-list">{product.keySpecifications.map((spec) => <li key={spec.label}>{spec.label}: {spec.value}</li>)}</ul><div className="cta-row"><Link className="button button-outline" href={`/products/${product.category}/${product.slug}`}>View Details</Link><Link className="button button-primary" href={`/request-a-quote?product=${product.slug}`}>Request a Quote</Link></div></article>)}</div><ProductPagination basePath={basePath} currentPage={page} totalPages={pageCount} /></div></section>
