...
export default async function ProductPage({ params }: PageProps) {
  ...
  return <>
      ...
      <article>
        <div className="product-showcase">
          <div className="product-gallery" aria-label={`${product.name} image gallery`}>
            {product.heroImage ? <Image className="product-detail-image" src={product.heroImage} alt={product.gallery[0]?.alt ?? `${product.name} product view`} width={1200} height={900} priority sizes="(max-width: 800px) 92vw, (max-width: 1180px) 58vw, 720px" /> : <div className="placeholder-image">Product image requires owner confirmation.</div>}
    ...
    <section className="section"><div className="content-wrap product-link-grid"><div><h2>Industry Solutions</h2><p>Review the operating context, available utilities and project constraints before final equipment selection.</p><Link className="button button-outline" href="/solutions">Explore Applications</Link></div><div><h2>Related Resources</h2><p>Use general planning resources as a starting point, then request model-specific documentation for procurement decisions.</p><Link className="button button-outline" href="/resources">View Technical Resources</Link></div></div></section>
