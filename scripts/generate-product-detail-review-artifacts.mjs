...
const rows = inventory.records.map((record) => {
  ...
  return {
    ...
    imageStatus: profile?.imageStatus ?? "Image requires owner confirmation",
...
const duplicates = rows.filter((row) => row.auditStatus === "duplicate"); const missingImages = rows.filter((row) => row.imageStatus !== "Authorized project image available"); const missingParameters = rows.filter((row) => row.parameterStatus === "Request verified specifications");
