...
export function Header() {
  ...
  return <>
    <header className="site-header">
      <SiteLogo />
      <nav className="desktop-nav" aria-label="Primary navigation">{primaryNavigation.map((item) => <Link key={item.href} href={item.href as Route}>{item.label}</Link>)}</nav>
      <div className="header-actions"><Link className="button button-primary" href="/request-a-quote">Get a Quote</Link><button ref={menuButtonRef} className="menu-button" type="button" aria-expanded={open} aria-controls="site-drawer" onClick={() => setOpen(true)}><span aria-hidden="true">☰</span> Menu</button></div>
    </header>
    ...
    <aside ref={drawerRef} id="site-drawer" className={`site-drawer ${open ? "open" : ""}`} role="dialog" aria-modal="true" aria-label="Site menu" tabIndex={-1}>
      ...
      <nav className="drawer-nav" aria-label="Menu navigation">{[...primaryNavigation.slice(1), { href: "/factory-quality", label: "Factory & Quality" }, { href: "/cases", label: "Cases" }].map((item) => <Link key={item.href} href={item.href as Route} onClick={() => setOpen(false)}>{item.label}</Link>)}</nav>
