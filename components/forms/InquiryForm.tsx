...
export function InquiryForm({ productModel, compact = false }: { productModel?: string; compact?: boolean }) {
  ...
  return <form className={`inquiry-form ${compact ? "compact" : ""}`} onSubmit={handleSubmit(onSubmit)} noValidate>
    ...
    <label>Product Category<select {...register("category")} aria-invalid={Boolean(errors.category)}><option value="">Select a category</option><option>Solar Light Towers</option><option>Site Monitoring Trailers</option><option>Diesel Light Towers</option></select>{errors.category && <span className="field-error" role="alert">{errors.category.message}</span>}</label>
